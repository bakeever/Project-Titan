...
*/
int getBearing(){
  // Trigger a reading from the compass sensor
  compass.read();

  // Get the raw azimuth value from the compass (0 to 359 degrees)
  int rawBearing = compass.getAzimuth();

  // Apply calibration offset to correct for compass mounting/orientation error
  // Offset = 293° (factory calibration error) + 45° (custom adjustment)
  const int calibrationOffset = 338;
  int adjustedBearing = (rawBearing + calibrationOffset) % 360;
  
  //Adjust for values over 360
  if(adjustedBearing>360){adjustedBearing=adjustedBearing-360;}
  if(adjustedBearing<0){adjustedBearing = adjustedBearing+360;}
  return 360-adjustedBearing;
}
/*
